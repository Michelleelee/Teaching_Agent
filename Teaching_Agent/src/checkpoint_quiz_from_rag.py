...
def main():
    ...
    for i, (uid, meta, score, dist, bm25_score) in enumerate(
        zip(
            res.ids,
            res.metadatas,
            res.hybrid_scores,
        ...
        print(f"[{i}] uid={uid} hybrid={score:.4f} vec_dist={dist_text} bm25={bm25_score:.4f} meta={meta}")
    print("-" * 80)

    # MVP：直接取 top1
    best_uid = res.ids[0]
    best_meta = res.metadatas[0]
